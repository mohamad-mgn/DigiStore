...
class OrderDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_queryset(self):
        # کاربر فقط سفارش‌های خودش را ببیند
...
class CheckoutView(LoginRequiredMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        try:
            order = OrderService.create_order_from_cart(user, address=address, postal_code=postal_code)
        except ValueError as e:
            messages.error(self.request, str(e))
            return redirect('cart:detail')

        # هدایت به صفحه پرداخت برای سفارش ایجادشده
...
class SellerOrderListView(LoginRequiredMixin, UserPassesTestMixin, ListView):
    ...
    def get_queryset(self):
        # سفارش‌هایی که حداقل یک آیتم از فروشنده دارند
